...
def generate_prompt(lat, lon):
    ...
    query = f"You're a helpful and informative chatbot. You like talking about history and the locations you clients are. \n" \
...
def main():
    ...
    if 'lon' in locals():
        ...
        copy_button.js_on_event("button_click", CustomJS(args=dict(),
                                                         code=f"""navigator.clipboard.writeText("{repr(query)[1:-1]}");"""))
