...
objs = nodes + ways
locs = []
filters = {"road", "footway", "path", 'highway', 'shop', 'barrier', 'railway', 'bus', 'created_by', 'emergency',
           'entrance', 'restaurant', 'abandoned:amenity', 'diplomatic',
           'disused:amenity', 'surveillance', 'fixme'}
for n in objs:
    if (len(n.tags) == 0 or len(filters.intersection(n.tags)) > 0 or \
        n.tags.get('amenity', None) in ['fuel', 'bicycle_parking', 'car_sharing', 'motorcycle_parking', 'taxi', 'bench',
                                        'atm', 'restaurant', 'cafe', 'drinking_water', 'bicycle_rental',
                                        'parking', 'theatre', 'bank', 'post_office', 'police', 'cinema', 'fast_food',
                                        'bar', 'toilets', 'ice_cream', 'pharmacy', 'railway'] or \
        n.tags.get('natural', None) == 'tree' or \
        n.tags.get('tourism', None) in ['hotel', 'guest_house'] or \
        not any('name' in t for t in n.tags)) \
